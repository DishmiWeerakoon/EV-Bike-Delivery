...
def run_baseline(scenario_name: str) -> dict:
    ...
    env.export_order_bike_table(os.path.join("results", "tables", f"orders_baseline_{scenario_name}.csv"))
    metrics = env.metrics()
    metrics["scenario"] = scenario_name
    metrics["policy"] = "baseline"
    return metrics
